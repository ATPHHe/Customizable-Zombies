...
local function tryShowModal()
    ...
    local data = CZ_Util.LoadFromFile("_data", true)
    if not data or data[1] == nil then 
        data = {} end
    if data[1] == CZ_Util.MOD_VERSION then return end
    
    local fContents = CZ_Util.LoadFromFile("_ReleaseNotes.txt", true)
    ...
    okModal(text, true, nil, nil, nil, nil, 
                function() 
                    ...
                    CZ_Util.SaveToFile("_data", text, true)
